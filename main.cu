...
__global__ void RandSolver(const short *d_lin_maze, int x_start, int y_start, int x_ext, int y_ext, int *flag, int n){
    ...
    if(idx < n){
        ...
        while(n_steps < 1000){  // You can choose between *flag != 1 OR n_steps < 1000
            n_steps++;
    ...
    */
    ...
    for (int i = 0; i < 8; ++i){
        for (int j = 0; j < 7; ++j){
            ...
            cudaEventRecord(t_start);
            RandSolver<<<(int)(n_particles[j] + blockSizes[i] -1)/blockSizes[i], blockSizes[i]>>>(d_lin_maze,
                                                                              solution->x,
                                                                              solution->y,start.x,
                                                                              start.y, d_flag,
                                                                              n_particles[j]);
    ...
    printf("\n");
    printf("TIMES TABLE [ms]\t number of step 1000\n");
    printf("-------------------------------------------------------------------------------\n");
    printf("|           |  N/20  |  N/10  |   N/5  |  2N/5  |    N/2   |  3N/4  |    N    |\n");
    printf("-------------------------------------------------------------------------------\n");

    for (int i = 1; i < 8; ++i){
        //if (i == 0) printf("| NTHR %d    |", blockSizes[i]);
        if (i<4 && i>0) printf("| NTHR %d   |", blockSizes[i]);
        if (i<7 && i>3) printf("| NTHR %d  |", blockSizes[i]);
        if (i == 7) printf("| NTHR %d |", blockSizes[i]);
